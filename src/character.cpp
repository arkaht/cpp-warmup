...
{
	std::cout << get_full_name() << " is a " << CharacterRaceStr[_race] << " specialized as a " << CharacterSpecialityStr[_speciality] << "! '" << _catch_phrase << "'" << std::endl;
}
...
void Character::equip_weapon( Weapon* weapon )
{
	...
	std::cout << get_full_name() << " equipped ";
	weapon->print_state();
}
